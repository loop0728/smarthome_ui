...
#define USE_AMIC					1
#define USE_DSPOTTER				1
#define COMMAND_LEN					40

//#define DSPOTTER_PATH_PREFIX		"/customer/testAD/DSpotter"
//#define DSPOTTER_LIB_PATH			"/customer/testAD/DSpotter/lib"
...
static char g_command[][COMMAND_LEN] = {
	"嗨, 小星星",	"打开电灯",	"关掉电灯",	"打开窗帘",
	"关闭窗帘",	"播放音乐",	"后一首",		"前一首",
	"停止音乐",	"打开空调",	"关闭空调",	"空调温度调高",
	"空调温度调低",	"清新模式",	"睡眠模式",	"开始煮饭",
	"风大一点",	"风小一点",	"音量调大",	"音量调小",
	"接通电话",	"开门",		"挂断电话",	"查看门口情况",
	"打开客厅的灯",	"关闭客厅的灯",	"离家 场景",	"降低 速度",
	"降低 坡度",	"提高 速度",	"提高 坡度",	"拍照",
	"今天天气",	"现在几点",	"开始",		"暂停",
	"洗涤模式",	"脱水模式",	"侵泡洗衣",	"轻柔模式",
	"强力模式",	"摩卡咖啡",	"拿铁咖啡",	"美式咖啡",
	"意式咖啡",	"卡布奇诺"
...
int SSTAR_VoiceDetectDeinit(HANDLE hDSpotter)
{
	...
	//DeinitTrainingWordList(&g_wordListHead);
...
static void *_SSTAR_VoiceAnalyzeProc_(void *pData)
{
    ...
    while (!g_stRecogThreadData.bExit)
    {
       ...
       if (s32Ret == DSPOTTER_SUCCESS)
       {
            ...
            {
                printf("Get command: id=%d, %s\n", nResultID, g_command[nResultID]);

                if (nResultID == 5 || nResultID == 8)
                {
